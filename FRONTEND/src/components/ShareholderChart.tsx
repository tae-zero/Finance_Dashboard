...
function ShareholderChart({ code, companyName }: ShareholderChartProps) {
  const [chartData, setChartData] = useState<ShareholderData | 'not-found' | 'error' | null>(null);
  ...
  useEffect(() => {
    fetch('/지분현황.json')
      ...
      .then(data => {
        ...
        const labels = rows.map((row: any) => row.주주구분?.trim() ?? '알 수 없음');
        const 지분율 = rows.map((row: any) => Number(row.지분율 ?? 0));
        const 대표주주수 = rows.map((row: any) => Number(row.대표주주수 ?? 0));
          ...
          datasets: [
            {
              ...
              datalabels: {
                ...
                formatter: (value: number) => `${value}%` // ✅ % 단위 표시
              }
            },
            {
              type: 'line',
              ...
              datalabels: {
                ...
                formatter: (value: number) => `${value.toLocaleString()}명` // ✅ 명 단위 표시
              }
            }
          ]
        }}
